// Enhanced AI Detection Coordinator with class filtering and AI-triggered recording
...
export class EnhancedAIDetectionCoordinator extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    logger.info('Initializing enhanced AI detection services...');
    ...
    logger.info('✓ Enhanced AI detection coordinator initialized');
  }
  ...
  private async performDetection(camera: Camera): Promise<void> {
    try {
      ...
      const filteredDetections = this.filterDetections(camera, detections);

      // Process filtered detections
      for (const detection of filteredDetections) {
        await this.saveDetection(camera, detection, snapshotPath);

        // Emit event for alarm system and recording trigger
        this.emit('detection', {
          cameraId: camera.id,
          detection,
        });

        // Trigger AI-based recording if configured
        await this.triggerAIRecording(camera, detection);
      }

      if (filteredDetections.length === 0 && fs.existsSync(snapshotPath)) {
  ...
  /**
   * Filter detections based on camera's filtered classes
   */
  private filterDetections(camera: Camera, detections: any[]): any[] {
    try {
      const filteredClasses = JSON.parse(camera.filteredDetectionClasses || '[]');

      // If no filter is set, return all detections
      ...
      return detections.filter(detection => {
        const shouldFilter = modelRegistry.shouldFilterClass(detection.label, filteredClasses);
        if (shouldFilter) {
          logger.debug(`Filtered out detection: ${detection.label} (not in allowed classes)`);
        }
        return !shouldFilter;
      });
    } catch (error) {
      logger.error('Error filtering detections:', error);
      return detections; // Return all on error
    }
  }

  /**
   * Trigger recording based on AI detection
   */
  private async triggerAIRecording(camera: Camera, detection: any): Promise<void> {
    try {
      const recordingMode = camera.recordingMode;

      // Check if AI-triggered recording is enabled
      if (recordingMode === 'ai_detection' || recordingMode === 'motion_or_ai') {
        // Only trigger if confidence is high enough
        if (detection.confidence >= camera.aiSensitivity / 100) {
          logger.info(`AI detection triggered recording on camera ${camera.name}: ${detection.label} (${(detection.confidence * 100).toFixed(1)}%)`);

          if (!recordingEngine.isRecording(camera.id)) {
            await recordingEngine.startRecording(camera, false); // AI-triggered, not motion
          }

          // Extend recording if already recording
          this.emit('ai:trigger', {
            cameraId: camera.id,
            detection,
          });
        }
      ...
      logger.error('Error triggering AI recording:', error);
    }
  }
  ...
  private async handleFrigateEvent(event: any): Promise<void> {
    try {
      ...
      // Apply filtering
      const filteredDetections = this.filterDetections(camera, [detection]);
...
export default new EnhancedAIDetectionCoordinator();
